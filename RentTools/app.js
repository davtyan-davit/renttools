document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelector(".slides").addEventListener("click", (event) => {
        if (event.target.classList.contains("slide")) {
            ...
            console.log("Slide clicked, src:", src);

            const match = src.match(/s(\d+)\.png/);
            if (!match) {
                console.error("No match found in src:", src);
                return;
            }
            ...
            openModal(images[index], names[index], prices[index]);
        }
    });
...
const names = [
    "Դռել Makita DHP482RTJ 18V Li-ion LXT Comi",
    "УШМ (բալգարկա) Bosch GWS 7 - 125",
    "Հարվածային գայկավյորտ XGT Makita TW007GZ",
    "Շինարարական խառնիչ Battipav Mini Mix 1000 (4100)",
    "Բալգարկա (անկյունային սրճաղաց) 180 մմ Makita GA7050",
    "Դռել 18V Li-ion 0-2400/MIN 150NM",
    "Պերֆորատոր Sturm 2512M",
    "Պերֆորատոր SDS-plus Makit HR 2470",
    "Վիբռոպլիտա",
    "Կոմպրեսոր 200 | 2 Zyl. 4,0 PS 400V"
];
...
function shop() {
    window.location.href = './օutlettools/index.html'
}
